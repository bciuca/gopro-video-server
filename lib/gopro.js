...
function getMedia(mediaPath) {
    ...
    return Rx.Observable.create(function(observer) {
        ...
        rxGetStream(url)
            .doAction(
                function downloadOnNext(data) {
                    outStream.write(data.chunk);
                }, 
                function downloadOnError(err) {
                    observer.onError(err);
                },
                function downloadOnCompleted() {
                    observer.onNext();
                    observer.onCompleted();
                }
            )
            .map(function downloadMap(data) {
            ...
            .doAction(
                function downloadProgress(progress) {
                    process.stdout.write('Downloading %d%\r', progress);
                }, 
                noop, 
                function downloadProgressCompleted() { 
                    console.log('done.')
                }
            )
            .subscribe();
        ...
        return getNthClip(-1)
            .map(function(clip) {
                ...
                captureObservable = startCapture()
                    ...
                    .delay(1000) // give gopro some time to finish writing to card?
                    .flatMap(function() {
